...
let config = { BASE_DIR: process.cwd(), DIR_PREFIX: '', PROJECT_MAP: {} };
...
function projectLabel(dir) {
  const prefix = config.DIR_PREFIX;
  if (prefix) {
    if (dir.startsWith(prefix + '-')) return dir.slice(prefix.length + 1);
    if (dir === prefix) return path.basename(config.BASE_DIR);
  }
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/prompt') {
    ...
    req.on('end', () => {
      try {
        ...
        const rootLabel = path.basename(config.BASE_DIR);
        const cwd = (!projectDir || projectDir === rootLabel)
          ? config.BASE_DIR
          : path.join(config.BASE_DIR, projectDir);
  ...
  // GET /config — serve client-side config (project map)
  if (req.method === 'GET' && req.url === '/config') {
    res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
    res.end(JSON.stringify({ projectMap: config.PROJECT_MAP || {} }));
    return;
  }
